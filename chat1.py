...
def process_data_magnitude(json_data):
    earthquake_counts = defaultdict(lambda: defaultdict(lambda: {'Low_Magnitude': 0, 'Medium_Magnitude': 0, 'High_Magnitude': 0}))
    
    for entry in json_data:
        # Extract date details
        date = entry['date']
        year, month, _ = date.split('-')  # Split the date into year, month, day
        magnitude = entry['magnitude']
        
        # Classify magnitude
        magnitude_type = classify_magnitude(magnitude)
        if magnitude_type:
            earthquake_counts[year][month][magnitude_type] += 1
    
    return earthquake_counts

def create_folder(folder_name):
    if os.path.exists(folder_name):
        shutil.rmtree(folder_name)  # Delete existing folder and all its contents
    os.makedirs(folder_name)  # Create a new folder

def prepare_plot_magnitude_data(earthquake_counts):
    plot_data = defaultdict(lambda: {'Low_Magnitude': [], 'Medium_Magnitude': [], 'High_Magnitude': []})
    
    for year, months in earthquake_counts.items():
        for month in range(1, 13):
            month_str = f'{month:02d}'  # Ensuring the month has two digits (01, 02, ..., 12)
            plot_data[year]['Low_Magnitude'].append(months.get(month_str, {}).get('Low_Magnitude', 0))
            plot_data[year]['Medium_Magnitude'].append(months.get(month_str, {}).get('Medium_Magnitude', 0))
            plot_data[year]['High_Magnitude'].append(months.get(month_str, {}).get('High_Magnitude', 0))
    
    return plot_data

def plot_magnitude_data(plot_data, folder_name):
    ...
    for year, data in plot_data.items():
        ...
        # Plot the lines for Low, Medium, and High Magnitude counts
        plt.plot(month_names, data['Low_Magnitude'], label='Low Magnitude', color='blue', marker='o')
        plt.plot(month_names, data['Medium_Magnitude'], label='Medium Magnitude', color='orange', marker='o')
        plt.plot(month_names, data['High_Magnitude'], label='High Magnitude', color='red', marker='o')
        
        # Set title, labels, and legend
        plt.title(f'Earthquake Magnitudes in {year}')
        plt.xlabel('Month')
        ...
        plt.savefig(f"{folder_name}/{year}_earthquake_magnitudes.png")
        plt.close()  # Close the plot to free memory

def process_rainfall_data(json_data):
    rainfall_counts = defaultdict(lambda: defaultdict(lambda: {'Low_rainfall': 0, 'Medium_rainfall': 0, 'High_rainfall': 0}))
    ...
    for entry in json_data:
        ...
        if rainfall_type:
            rainfall_counts[year][month][rainfall_type] += 1
    
    return rainfall_counts

def prepare_plot_rainfall_data(rainfall_counts):
    plot_data = defaultdict(lambda: {'Low_rainfall': [], 'Medium_rainfall': [], 'High_rainfall': []})
    
    for year, months in rainfall_counts.items():
        for month in range(1, 13):
            month_str = f'{month:02d}'  # Ensuring the month has two digits (01, 02, ..., 12)
            plot_data[year]['Low_rainfall'].append(months.get(month_str, {}).get('Low_rainfall', 0))
            plot_data[year]['Medium_rainfall'].append(months.get(month_str, {}).get('Medium_rainfall', 0))
            plot_data[year]['High_rainfall'].append(months.get(month_str, {}).get('High_rainfall', 0))
    
    return plot_data

def plot_rainfall_data(plot_data, folder_name):
    # Ensure the folder exists
    create_folder(folder_name)
    month_names = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
    for year, data in plot_data.items():
        ...
        # Plot the lines for Low, Medium, and High Magnitude counts
        plt.plot(month_names, data['Low_rainfall'], label='Medium_rainfall', color='blue', marker='o')
        plt.plot(month_names, data['Medium_rainfall'], label='Medium_rainfall', color='orange', marker='o')
        plt.plot(month_names, data['High_rainfall'], label='High_rainfall', color='red', marker='o')
        ...
        # Save the plot as a PNG file in the specified folder
        plt.savefig(f"{folder_name}/{year}_rainfall.png")
        plt.close()  # Close the plot to free memory
...
def process_precipitation_total(json_data):
    records = []
    for entry in json_data:
        date = entry["date"]
        year, month, day = date.split("-")
        rain_sum = entry["weather"]["rain_sum"]
        snowfall_sum = entry["weather"]["snowfall_sum"]
        precipitation_sum = rain_sum + snowfall_sum  # Calculate the precipitation sum
        records.append({"year": year, "month": month, "precipitation_sum": precipitation_sum})
    return records

def prepare_plot_precipitation_data(records):
    df = pd.DataFrame(records)

# Convert month to a short month name (e.g., 'Jan', 'Feb')
    df['month'] = pd.to_datetime(df['month'], format='%m').dt.month_name().str[:3]

# Group by year and month and calculate the total precipitation_sum for each month of each year
    df_avg = df.groupby(['year', 'month'])['precipitation_sum'].sum().reset_index()
    return df_avg

def plot_save_precepitation_data(df_avg ,folder_name):
    create_folder(folder_name)
    for year in df_avg['year'].unique():
        df_year = df_avg[df_avg['year'] == year]
        
        # Create a plot for each year
        plt.figure(figsize=(10, 6))
        plt.plot(df_year['month'], df_year['precipitation_sum'], marker='o', color='b', linestyle='-', linewidth=2)
        
        plt.title(f'Precipitation Sum (Rain + Snowfall) Over Months - Year {year}', fontsize=16)
        plt.xlabel('Month', fontsize=12)
        plt.ylabel('Precipitation Sum (Rain + Snowfall)', fontsize=12)
        plt.grid(True)

        # Save the plot as an image for the current year
        file_path = os.path.join(folder_name, f'precipitation_sum_{year}.png')
        plt.savefig(file_path)
        plt.close()  # Close the plot to avoid overlap in the next one

def process_data_magnitude_rain(json_data):
    earthquake_counts = defaultdict(lambda: defaultdict(lambda: {'Low_Magnitude': 0, 'Medium_Magnitude': 0, 'High_Magnitude': 0}))
    rainfall_counts = defaultdict(lambda: defaultdict(lambda: {'Low_rainfall': 0, 'Medium_rainfall': 0, 'High_rainfall': 0}))
    ...
    for entry in json_data:
        ...
        year, month, _ = date.split('-')  # Split the date into year, month, day
        magnitude = entry['magnitude']
        rainfall = entry.get('weather', {}).get('rain_sum', 0)  # Total rainfall for the day
        ...
        # Classify rainfall
        if rainfall <= 5:
            rainfall_counts[year][month]['Low_rainfall'] += 1
        elif 6 <= rainfall <= 10:
            rainfall_counts[year][month]['Medium_rainfall'] += 1
        elif rainfall >= 10:
            rainfall_counts[year][month]['High_rainfall'] += 1
    
    return earthquake_counts, rainfall_counts

def prepare_plot_magnitude_rain_data(earthquake_counts, rainfall_counts):
    plot_data = defaultdict(lambda: {
        'Low_Magnitude': [], 'Medium_Magnitude': [], 'High_Magnitude': [],
        'Low_rainfall': [], 'Medium_rainfall': [], 'High_rainfall': [], 'Total_rainfall': []
    })
    ...
    for year, months in earthquake_counts.items():
        for month in range(1, 13):
            ...
            # Rainfall data
            plot_data[year]['Low_rainfall'].append(rainfall_counts[year].get(month_str, {}).get('Low_rainfall', 0))
            plot_data[year]['Medium_rainfall'].append(rainfall_counts[year].get(month_str, {}).get('Medium_rainfall', 0))
            plot_data[year]['High_rainfall'].append(rainfall_counts[year].get(month_str, {}).get('High_rainfall', 0))
            
            # Total rainfall (sum of low, medium, and high rainfall)
            total_rainfall = (plot_data[year]['Low_rainfall'][-1] + 
                              plot_data[year]['Medium_rainfall'][-1] + 
                              plot_data[year]['High_rainfall'][-1])
            plot_data[year]['Total_rainfall'].append(total_rainfall)
    
    return plot_data

def plot_magnitude_Rain_data(plot_data, folder_name):
    # Ensure the folder exists
    create_folder(folder_name)
    month_names = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
    ...
    for year, data in plot_data.items():
        ...
        # Plot the lines for Low, Medium, and High Magnitude counts
        ax1.plot(month_names, data['Low_Magnitude'], label='Low Magnitude', color='blue', marker='o')
        ax1.plot(month_names, data['Medium_Magnitude'], label='Medium Magnitude', color='orange', marker='o')
        ax1.plot(month_names, data['High_Magnitude'], label='High Magnitude', color='red', marker='o')
        ax1.set_xlabel('Month')
        ax1.set_ylabel('Earthquake Frequency', color='black')
        ax1.tick_params(axis='y', labelcolor='black')
        
        # Create a second y-axis to plot total rainfall
        ax2 = ax1.twinx()
        ax2.plot(month_names, data['Total_rainfall'], label='Total Rainfall', color='green', marker='x')
        ax2.set_ylabel('Total Rainfall', color='green')
        ax2.tick_params(axis='y', labelcolor='green')
        
        # Set title and grid
        plt.title(f'Earthquake Magnitudes and Total Rainfall in {year}')
        ax1.grid(True)
        ...
        # Save the plot as a PNG file in the specified folder
        plt.savefig(f"{folder_name}/{year}_earthquake_magnitudes_rainfall.png")
        plt.close()  # Close the plot to free memory

def process_data_rain_precipitation(json_data):
    rainfall_counts = defaultdict(lambda: defaultdict(lambda: {'Low_rainfall': 0, 'Medium_rainfall': 0, 'High_rainfall': 0}))
    precipitation_hours = defaultdict(lambda: defaultdict(list))  # Store precipitation hours per month
    ...
    for entry in json_data:
        ...
        year, month, _ = date.split('-')  # Split the date into year, month, day
        rainfall = entry.get('weather', {}).get('rain_sum', 0)
        precip_hours = entry.get('weather', {}).get('precipitation_hours', 0)
        ...
        # Collect precipitation hours
        precipitation_hours[year][month].append(precip_hours)
    
    # Calculate average precipitation hours for each month
    avg_precip_hours = defaultdict(lambda: defaultdict(float))
    for year, months in precipitation_hours.items():
        for month, hours in months.items():
            avg_precip_hours[year][month] = sum(hours) / len(hours) if hours else 0
    
    return rainfall_counts, avg_precip_hours

def prepare_rainfall_precipitation_plot_data(rainfall_counts, avg_precip_hours):
    plot_data = defaultdict(lambda: {'Low_rainfall': [], 'Medium_rainfall': [], 'High_rainfall': [], 'Avg_precip_hours': []})
    ...
    for year, months in rainfall_counts.items():
        for month in range(1, 13):
            ...
            plot_data[year]['Medium_rainfall'].append(months.get(month_str, {}).get('Medium_rainfall', 0))
            plot_data[year]['High_rainfall'].append(months.get(month_str, {}).get('High_rainfall', 0))
            plot_data[year]['Avg_precip_hours'].append(avg_precip_hours[year].get(month_str, 0))
    
    return plot_data

def plot_rainfall_precipitation_data(plot_data, folder_name):
    # Ensure the folder exists
    create_folder(folder_name)
    month_names = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
    ...
    for year, data in plot_data.items():
        ...
        # Plot the lines for Low, Medium, and High rainfall counts
        ax1.plot(month_names, data['Low_rainfall'], label='Low Rainfall', color='blue', marker='o')
        ax1.plot(month_names, data['Medium_rainfall'], label='Medium Rainfall', color='orange', marker='o')
        ax1.plot(month_names, data['High_rainfall'], label='High Rainfall', color='red', marker='o')
        ax1.set_xlabel('Month')
        ax1.set_ylabel('Rainfall Frequency', color='black')
        ax1.tick_params(axis='y', labelcolor='black')
        
        # Create a second y-axis to plot average precipitation hours
        ax2 = ax1.twinx()
        ax2.plot(month_names, data['Avg_precip_hours'], label='Avg Precipitation Hours', color='green', marker='x')
        ax2.set_ylabel('Avg Precipitation Hours', color='green')
        ax2.tick_params(axis='y', labelcolor='green')
        
        # Set title and grid
        plt.title(f'Rainfall and Precipitation Hours in {year}')
        ax1.grid(True)
        ...
        # Save the plot as a PNG file in the specified folder
        plt.savefig(f"{folder_name}/{year}_rainfall_precipitation.png")
        plt.close()  # Close the plot to free memory

def get_sunshine1_duration(sunshine_seconds):
...
def calculate_highest_lowest_sunshine_per_year_and_month(data):
    ...
    for entry in data:
        ...
        sunshine_hours = get_sunshine1_duration(sunshine_seconds)
...
def calculate_magnitude_and_elevation_per_year_and_month(json_data):
...
def plot_and_save_magnitude_elevation_line_bar_graph(magnitude_elevation_data, elevation_classes, folder_path):
...
def generate_response(input_text):
    ...
    if "Line_graph_view_general_data" in input_text.lower():
        print("I can show you global earthquake data in the the following spects below choose how you would like to proceed:")
        print("1. earthquake magnitude line graph")
        print("2. Rainfall (rain_sum) line graph")
        print("3. Max ,min,mean tempreture graph")
        print("4. precepitation_graph")
        print("5. rainfall vs magnitude line")
        print("6. Rainfall vs precipitation")
        print("7. sun_light_line")
        ...
        if search_type == '1':
            ...
            if merge_json_files:
               ...
               json_data = load_json_data(file_path)
            if json_data:
               process_data_magnitude(json_data) 
               prepare_plot_magnitude_data(earthquake_counts)
               folder_name = 'earthquake_Line_graph'
               plot_magnitude_data(plot_data, folder_name)
            if plot_magnitude_data:
            ...
            if merge_json_files:
               ...
               json_data = load_json_data(file_path)
            if json_data:
                rainfall_counts = process_rainfall_data(json_data)
                plot_data = prepare_plot_rainfall_data(rainfall_counts)          
                folder_name = 'rainfall_plots'
                plot_rainfall_data(plot_data, folder_name)
            if plot_rainfall_data:
               print(f"Plots saved to folder: {folder_name}")
            ...
            if merge_json_files:
               ...
               json_data = load_json_data(file_path)
            if json_data:
                folder_path="tempreture_line_graphs"
            ...
            if plot_and_save_temperature_averages:
            ...
            if merge_json_files:
               ...
               json_data = load_json_data(file_path)
            if json_data:
                folder_name ="precipitation_sum_line_distribution"
                records=  process_precipitation_total(json_data)
                df_avg= prepare_plot_precipitation_data(records)
            if prepare_plot_precipitation_data:
                plot_save_precepitation_data(df_avg ,folder_name)
            if plot_save_precepitation_data:
            ...
            if merge_json_files:
               ...
               json_data = load_json_data(file_path)
            if json_data:
                folder_name = 'earthquake_rainfall_plots'
                earthquake_counts, rainfall_counts = process_data_magnitude_rain(json_data)
                plot_data = prepare_plot_magnitude_rain_data(earthquake_counts, rainfall_counts)
                plot_magnitude_Rain_data(plot_data, folder_name)
            ...
            if merge_json_files:
                    ...
                    json_data = load_json_data(file_path)
            if json_data:
                    rainfall_counts, avg_precip_hours = process_data_rain_precipitation(json_data)
                    plot_data = prepare_rainfall_precipitation_plot_data(rainfall_counts, avg_precip_hours)
                    plot_rainfall_precipitation_data(plot_data, folder_name)
            ...
            if merge_json_files:
                    ...
                    json_data = load_json_data(file_path)
            if json_data:
                    folder_path="sunshine_folder"
                    highest_lowest_sunshine = calculate_highest_lowest_sunshine_per_year_and_month(json_data)
                    plot_and_save_sunshine(highest_lowest_sunshine, folder_path)

        else:
            return "Invalid choice. Please enter 1, 2, or 3."

        return f"Search completed. line_graphs are ready check display files"
    
    if "Line_graph_view_general_data_place" in input_text.lower():
        print("I can show you global earthquake data in the the following spects below choose how you would like to proceed:")
        print("1. earthquake magnitude line graph")
        print("2. Rainfall (rain_sum) line graph")
        print("3. Max ,min,mean tempreture graph")
        ...
        if search_type == '1':
            date_rang_start = (input("Enter start date (YYYY-MM-DD): "))
            date_rang_end =(input("Enter end date (YYYY-MM-DD): "))
            if date_rang_start and date_rang_end:
                user_place=(input("enter place for choosing"))
                singleplaceatom("user.atomfiles",date_range_start,date_range_end,user_place,"displayfiles\\earthquake_data.json")
            if singleplaceatom:
               display_weather_by_date_range("weather_userdata",date_range_start,date_range_end,"displayfiles\\filtered_weather.json")
            ...
            if merge_json_files:
               ...
               json_data = load_json_data(file_path)
            if json_data:
               earthquake_counts = process_data_magnitude(json_data) 
               plot_data = prepare_plot_magnitude_data(earthquake_counts)
            if plot_data:
                folder_name = 'earthquake_Line_graph'
                plot_magnitude_data(plot_data, folder_name)
            if plot_magnitude_data:
